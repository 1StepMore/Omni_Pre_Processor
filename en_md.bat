...
REM Usage: en_md.bat "file" [flags]
REM Supports drag-drop of files and folders

if "%~1"=="" (
    echo OPP Markdown Converter
    echo.
    echo Usage: en_md.bat "file" [flags]
    echo         en_md.bat "folder" [flags]
    echo.
    echo Converts document to Markdown format with English OCR support.
    ...
    echo.
    echo Example:
    echo   en_md.bat "document.docx"
    echo   en_md.bat "folder" --output-dir ./output
    pause
    exit /b 1
)
...
REM Set OCR language to English for PDF images
set OPP_OCR_LANG=eng
