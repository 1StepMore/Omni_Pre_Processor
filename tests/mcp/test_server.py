...
from tests.mcp.conftest import get_data
...
class TestExtractDocumentTool:
    ...
    async def test_extract_document_valid_docx(self, setup_server):
        ...
        assert get_data(result)["success"] is True
        assert "data" in get_data(result) or "md_content" in get_data(result)
    ...
    async def test_extract_document_path_traversal(self, setup_server):
        ...
        assert get_data(result)["success"] is False
        assert "error" in get_data(result)
    ...
    async def test_extract_document_invalid_format(self, setup_server):
        ...
        assert get_data(result)["success"] is False
        assert "Invalid output format" in get_data(result).get("error", "")
...
class TestBatchExtractTool:
    ...
    async def test_batch_extract_all_valid(self, setup_server):
        ...
        assert get_data(result)["success"] is True
        assert "results" in get_data(result)
        assert get_data(result)["successful"] >= 1
    ...
    async def test_batch_extract_fail_fast_on_invalid(self, setup_server):
        ...
        assert get_data(result)["success"] is False
        assert "validation_errors" in get_data(result) or "error" in get_data(result)
...
class TestDetectFormatTool:
    ...
    async def test_detect_format_known_formats(self, setup_server, filename: str, expected_format: str):
        ...
        assert get_data(result)["success"] is True
        assert get_data(result)["format"] == expected_format
        assert "confidence" in get_data(result)
...
class TestGenerateMarkdownTool:
    ...
    async def test_generate_markdown_content_generation(self, setup_server):
        ...
        assert get_data(result)["success"] is True
        assert "markdown_content" in get_data(result)
        assert len(get_data(result)["markdown_content"]) > 0
...
class TestGenerateXliffTool:
    ...
    async def test_generate_xliff_docx(self, setup_server):
        ...
        if get_data(result)["success"]:
            assert "xliff_content" in get_data(result)
            assert len(get_data(result)["xliff_content"]) > 0
            assert "units_count" in get_data(result)
        else:
            assert "error" in get_data(result)
    ...
    async def test_generate_xliff_pdf_error(self, setup_server):
        ...
        if not get_data(result)["success"]:
            assert "error" in get_data(result)
    ...
    async def test_generate_xliff_units_count(self, setup_server):
        ...
        if get_data(result)["success"]:
            assert get_data(result)["units_count"] >= 0
            trans_unit_count = get_data(result)["xliff_content"].count("<trans-unit")
            assert get_data(result)["units_count"] == trans_unit_count
...
class TestServerInitialization:
    ...
    def test_tool_functions_return_dicts_not_exceptions(self, tmp_path: Path):
        ...
        try:
            ...
            assert isinstance(get_data(result), dict)
            assert "success" in get_data(result)
