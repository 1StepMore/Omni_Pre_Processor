...
def _align_skeleton_with_xliff_ids(skeleton_path: Path, xliff_path: Path) -> None:
    ...
    a text-to-id map, then update the skeleton's ``data-trans-unit-id`` attributes
...
def process_single_file(
    ...
    try:
        ...
        if skeleton_path:
            ...
            # list in different orders, so the IDs in the skeleton don't match
            # the XLIFF's. Post-process by matching element text to XLIFF source.
            if (
                proc_result.format_type == FormatType.EPUB
